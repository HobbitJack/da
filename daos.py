...
class DAOS:
    # Procedure
    def __init__(self, functions: dict[str, tuple[int, callable, str]]) -> None:
        self.operation_stack: list[str] = []
        self.item_stack: typing.Any = []
        self.current_item = ""

        self.variables = {"E": mpmath.e, "PI": mpmath.pi}
        self.operation_arguments = {"=": 0, "+": 2, "-": 2, "*": 2, "/": 2, "^": 2}
        self.operation_priorities = {"=": 5, "(": 5, "+": 4, "-": 4, "*": 3, "/": 3, "^": 2}
    ...
    def evaluate_expression(self, expression) -> None:
        ...
        for character in expression:
            if character in self.operation_arguments:
                ...
                while len(self.operation_stack) >= 1 and self.operation_priorities[self.operation_stack[-1]] <= self.operation_priorities[character]:
    ...
    def evaluate_last_operation(self) -> None:
        arguments = [self.item_stack[i] for i in range(len(self.item_stack) - self.operation_arguments[self.operation_stack[-1]], len(self.item_stack))]
        for _ in arguments:
            self.item_stack.pop()
    ...
    def evaluate_current_item(self) -> None:
        if self.current_item:
            if self.current_item.upper() not in self.operation_arguments:
                self.item_stack.append(self.variables[self.current_item] if self.current_item in self.variables.keys() else mpmath.mpf(self.current_item))
            else:
...
if __name__ == "__main__":
    daos_instance = DAOS({"SIN": (1, lambda arguments : mpmath.sin(arguments[0])), "COS": (1, lambda arguments : mpmath.cos(arguments[0])), "TAN": (1, lambda arguments : mpmath.tan(arguments[0])), "SQRT": (1, lambda arguments : mpmath.sqrt(arguments[0])), "LN": (1, lambda arguments : mpmath.ln(arguments[0])), "EXP": (1, lambda arguments : mpmath.exp(arguments[0])), "RAD": (1, lambda arguments : arguments[0] * mpmath.pi / 180), "DEG": (1, lambda arguments : arguments[0] * 180 / mpmath.pi)})
    expression = "="
    expression += input("=")
